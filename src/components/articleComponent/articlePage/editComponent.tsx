...
const updateArticle = async (
  ...
  const res = await fetch(`/api/articles/${data.get("id")}`, {
    headers: { "Content-Type": "application/json" },
    method: "PUT",
    body: JSON.stringify({
      id: data.get("id"),
...
export default function ArticleEdit({ article }: { article: Article }) {
  ...
  const [isEnablePublic, setIsEnablePublic] = useState(Boolean(article?.visible & 0x1));
  const [longDescriptionState, setLongDescriptionState] = useState(article?.main_text);
  ...
  useEffect(() => {
    if (formState.status === "success" || formState.status === "reflesh") {
      redirect(`/articles/${article.id}/edit?status=success&message=${formState.message}`);
    }
    if (formState.status === "error") {
      redirect(`/articles/${article.id}/edit?status=error&message=${formState.message}`);
    }
  }, [formState.status]);
        ...
        <>
          ...
          >
              ...
              <ThemeProvider theme={formTheme}>
                ...
                >
                  ...
                  <MuiFileInput
                    ...
                    value={file}
                  />
                  {article.image && article.image_file && (
                    <>
                      ...
                      <Button
                        onClick={async () => {
                          const res = await fetch(
                            `/api/images?filename=${article.image_file}&clubId=${article.id}`,
                            {
                              method: "DELETE",
                            }
                          );
                          if (res.ok) {
                            ...
                            if (res.ok)
                              window.location.href = `/articles/${article.id}/edit?status=success&message=画像を削除しました。`;
                            else
                              window.location.href = `/articles/${article.id}/edit?status=error&message=画像の削除に失敗しました。`;
                          } else
                            window.location.href = `/articles/${article.id}/edit?status=error&message=画像の削除に失敗しました。`;
                        }}
                        color="error"
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          name="internal"
                          defaultChecked={
                            article?.visible ? (article?.visible & 0x1) == 0x1 : false
                          }
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          name="public"
                          defaultChecked={
                            article?.visible ? (article?.visible & 0x2) == 0x2 : false
                          }
                  ...
                  <TextField
                    ...
                    fullWidth
                    margin="normal"
                    defaultValue={article.main_text}
                    multiline
                    minRows={10}
                    maxRows={10}
            ...
            >
              ...
              <AlertDialog id={article.id.toString()} />
            </Stack>
          </Stack>
        </>
      ) : (
