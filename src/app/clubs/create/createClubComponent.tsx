  ...
  return (
    ...
    >
      ...
      <Typography variant="body1">
        同好会を登録することで、Linkleを使用した同好会検索に掲載されるようになります。
      </Typography>
      <Stack
        spacing={2}
        bgcolor={"lightgray"}
      ...
      <ThemeProvider theme={formTheme}>
        ...
        >
          ...
          >
            ...
            />
            <FormHelperText>
              学園側に届け出ている名称です。(仮)や○○同好会の同好会までご記入ください。
            </FormHelperText>
            <Typography
              variant="h6"
              mt={3}
            ...
            />
            {chutobu || kotobu ? null : (
              <FormHelperText error>どちらか一方または両方を選択してください。</FormHelperText>
            )}
            <Typography
              variant="h6"
              mt={3}
            ...
            {slack_link ? (
              <>
                {slack_link.match(/https:\/\/.*\.slack\.com\/archives\/.*/) ? null : (
                  <FormHelperText error>
                    httpsから始まるSlackのチャンネルリンクを入力してください。
                  </FormHelperText>
                )}
              </>
            ) : (
              ...
              render={({ field }) => (
                <FormControl error={Boolean(field.value) === false}>
                  <FormControlLabel
                    required
                    control={<Checkbox {...field} />}
                    label={<>私はこの同好会の管理者または代表者です。</>}
                  />
                </FormControl>
              )}
            />
            <FormHelperText>
              管理者・代表者でない場合、この登録を取り消したり、あなたのアカウントを停止する場合があります。
            </FormHelperText>
            <Button
              type="submit"
              variant="contained"
