...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (session && !(user_clubData.includes(session?.user?.email || "")) && !((clubData.visible & 0x1) == 0x1)) return NextResponse.json({ status: 403 });
    return Response.json(clubData);
}

export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    const owners = (await (await fetch(`${endpoint}/user_club/?filter1=club,eq,${id}`)).json()).records.map((record: { user: string }) => record.user);
    const session = await auth();
